  ...
  {	$headers = getallheaders();
  	...
  	if(!isset($headers['x-auth-token']) && !empty($headers['x-auth-token'])){
  		...
  		try {        // decode jwt
		  	...
		  	return json_encode(array("status" => "ok", "message" => "Informacion recuperada con exito.",  "data" => $curso, "codigo"=> "200"));
		  	//return $curso;
		  	}
		  	// if decode fails, it means jwt is invalid
		catch (Exception $e){		  		
		  		return json_encode(array("status" => "error", "message" => $e->getMessage(), "codigo"=> "401"));
		  	}
